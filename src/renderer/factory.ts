...
export type CreatePageConstructorContentFn = (content: PageContent) => string;
...
export function getPageConstructorContent(content: PageContent): string {
    if (!currentImplementation) {
        throw new Error('Page constructor implementation not set. Call setPageConstructorImplementation first.');
    }
    
    return currentImplementation(content);
}